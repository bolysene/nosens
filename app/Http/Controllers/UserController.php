...
class UserController extends Controller
{
    ...
    public function changePasswordPost(ChangePasswordRequest $request)
    {
        if (Hash::check($request->old_password, Auth::user()->password)) {
            ...
            return back()->with('status', 'Nouveau mot de passe enregistré avec succès');
        }

        return back()->with('error-status', 'Les mots de passes ne correspondent pas.');
    }
    ...
    public function store(CreateUserRequest $request)
    {
        DB::transaction(function () use ($request) {
            ...
            $tb->student_code = "SN-".Auth::user()->id.'-'.date("d")."-".date('Y')."-".rand(0,9000000);
        ...
        return back()->with('status', $request->name.' enregistré avec succès');
    }
    ...
    public function storeAdmin(CreateAdminRequest $request)
    {
        ...
        $tb->student_code = "Admin-".date("d")."-".date('Y')."-".rand(0,100);
        $tb->gender = $request->gender;
        $tb->blood_group = $request->blood_group;
        $tb->nationality = (!empty($request->nationality)) ? $request->nationality : '';
        $tb->phone_number = $request->phone_number;
        ...
        return back()->with('status', $request->name.' a été enregistré comme Admin de votre établissement');
    }
